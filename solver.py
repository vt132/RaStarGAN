...
class Solver(object):
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            d_loss_cls = self.classification_loss(out_cls_real, label_org, self.dataset)
            ...
            if self.use_rad:
                # Relativistic Average Discriminator Loss (BCE-based)
                d_loss_real = F.mse_loss(
                    out_src_real - out_src_fake.mean(), torch.ones_like(out_src_real))
                d_loss_fake = F.mse_loss(
                    out_src_fake - out_src_real.mean(), torch.zeros_like(out_src_fake))
            else:
                d_loss_real = - torch.mean(out_src_real)
                d_loss_fake = torch.mean(out_src_fake)

            # Gradient penalty (disabled for RaD)
            if lambda_gp > 0:
                ...
                d_loss_gp = torch.tensor(0.0).to(self.device)

            # Combine discriminator loss
            d_loss = d_loss_real + d_loss_fake + self.lambda_cls * d_loss_cls + lambda_gp * d_loss_gp
            self.reset_grad()
            d_loss.backward()
            ...
            loss = {
                ...
                'D/loss_gp': d_loss_gp.item() if lambda_gp > 0 else 0.0,
            }
            # =================================================================================== #
            ...
            if (i+1) % self.n_critic == 0:
                ...
                g_loss_cls = self.classification_loss(out_cls_fake, label_trg, self.dataset)

                if self.use_rad:
                    with torch.no_grad():
                        out_src_real, _ = self.D(x_real.detach())

                    # Relativistic Average Generator Loss (BCE-based)
                    g_loss_fake = F.mse_loss(
                        out_src_fake - out_src_real.mean(dim=0, keepdim=True),
                        torch.ones_like(out_src_fake)
                    ) + F.mse_loss(
                        out_src_real - out_src_fake.mean(dim=0, keepdim=True),
                        torch.zeros_like(out_src_real)
                    )
    ...
    def train_multi(self):
        ...
        for i in range(start_iters, self.num_iters):
            for dataset in ['CelebA', 'RaFD']:
                ...
                if self.use_rad:
                    d_loss_real = torch.mean((out_src - torch.mean(out_src_fake) - 1) ** 2)
                    d_loss_fake = torch.mean((out_src_fake - torch.mean(out_src) + 1) ** 2)
                else:
                ...
                if (i+1) % self.n_critic == 0:
                    ...
                    if self.use_rad:
                        with torch.no_grad():
                            out_src_real, _ = self.D(x_real)
                        # The generator loss should only make fakes look more real than real images.
                        # The second term was incorrect and caused conflicting gradients.
                        g_loss_fake = torch.mean((out_src - torch.mean(out_src_real) + 1) ** 2)
                    else:
